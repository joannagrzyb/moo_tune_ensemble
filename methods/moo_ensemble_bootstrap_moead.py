...
from methods.optimization_param_moead import OptimizationParamMOEAD
...
class MooEnsembleSVCbootstrapMOEAD(BaseEstimator):

    def __init__(self, base_classifier, scale_features=0.5, n_repeats=5, test_size=0.5, objectives=2, p_size=100, predict_decision="ASV", p_minkowski=2):
    ...
    def partial_fit(self, X, y, classes=None):
        ...
        for repeat in range(self.n_repeats):
            if repeat == 0:
                ...
                problem = OptimizationParamMOEAD(bs_X, bs_y, test_size=self.test_size, estimator=self.base_classifier, scale_features=self.scale_features, n_features=n_features, objectives=self.objectives)
                # n_var=n_variable,
                ...
                if res.X is None:
                    ...
                    for result_opt in res.X:
                        ...
                        # Add candidate to the ensemble
                        self.ensemble.append(candidate)
        print("Solutions:", self.solutions, "Result:", res.X)
        return self
