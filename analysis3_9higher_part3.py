...
for dataset_id, dataset in enumerate(find_datasets(DATASETS_DIR)):
    ...
    for clf_id, clf_name in enumerate(methods):
        for metric_id, metric in enumerate(metrics_alias):
            try:
                filename = "results/experiment_server/experiment3_9higher_part3/raw_results/%s/%s/%s.csv" % (metric, dataset, clf_name)
...
def horizontal_bar_chart():
    for metric_id, metric in enumerate(metrics_alias):
        ...
        # Save plot
        filename = "results/experiment_server/experiment3_9higher_part3/plot_bar/ex3_bar_%s" % (metric)
        if not os.path.exists("results/experiment_server/experiment3_9higher_part3/plot_bar/"):
            os.makedirs("results/experiment_server/experiment3_9higher_part3/plot_bar/")
...
pairs_metrics_multi(method_names=methods_alias, data_np=data_np, experiment_name="experiment_server/experiment3_9higher_part3", dataset_names=datasets, metrics=metrics_alias, filename="ex3_ranking_plot", ref_method=methods_alias[0])

pairs_metrics_multi(method_names=methods_alias, data_np=data_np, experiment_name="experiment_server/experiment3_9higher_part3", dataset_names=datasets, metrics=metrics_alias, filename="ex3_ranking_plot", ref_method=methods_alias[1])
