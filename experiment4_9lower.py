...
logging.basicConfig(filename='devlogs/experiment4_9lower.log', filemode="a", format='%(asctime)s - %(levelname)s: %(message)s', level='DEBUG')
logging.info("--------------------------------------------------------------------------------")
        ...
        for fold_id in range(n_folds):
            for sol_id in range(n_rows_p):
                if (pareto_solutions[fold_id, sol_id, 0] != 0.0) and (pareto_solutions[fold_id, sol_id, 1] != 0.0):
                    filename_pareto = "results/experiment4_9lower/pareto_raw/%s/fold%d/sol%d.csv" % (dataset, fold_id, sol_id)
                    if not os.path.exists("results/experiment4_9lower/pareto_raw/%s/fold%d/" % (dataset, fold_id)):
                        os.makedirs("results/experiment4_9lower/pareto_raw/%s/fold%d/" % (dataset, fold_id))
