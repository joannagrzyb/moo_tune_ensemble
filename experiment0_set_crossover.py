...
logging.basicConfig(filename='textinfo/experiment0_set_crossover.log', filemode="a", format='%(asctime)s - %(levelname)s: %(message)s', level='DEBUG')
logging.info("--------------------------------------------------------------------------------")
    ...
    try:
        ...
        for clf_id, clf_name in enumerate(methods):
            for metric_id, metric in enumerate(metrics_alias):
                # Save metric results
                filename = "results/experiment0_set_crossover/raw_results/%s/%s/%s.csv" % (metric, dataset, clf_name)
                if not os.path.exists("results/experiment0_set_crossover/raw_results/%s/%s/" % (metric, dataset)):
                    os.makedirs("results/experiment0_set_crossover/raw_results/%s/%s/" % (metric, dataset))
                np.savetxt(fname=filename, fmt="%f", X=scores[metric_id, clf_id, :])
            # Save diversity results
            filename = "results/experiment0_set_crossover/diversity_results/%s/%s.csv" % (dataset, clf_name)
            if not os.path.exists("results/experiment0_set_crossover/diversity_results/%s/" % (dataset)):
                os.makedirs("results/experiment0_set_crossover/diversity_results/%s/" % (dataset))
        ...
        for fold_id in range(n_folds):
            for sol_id in range(n_rows_p):
                if (pareto_solutions[fold_id, sol_id, 0] != 0.0) and (pareto_solutions[fold_id, sol_id, 1] != 0.0):
                    filename_pareto = "results/experiment0_set_crossover/pareto_raw/%s/fold%d/sol%d.csv" % (dataset, fold_id, sol_id)
                    if not os.path.exists("results/experiment0_set_crossover/pareto_raw/%s/fold%d/" % (dataset, fold_id)):
                        os.makedirs("results/experiment0_set_crossover/pareto_raw/%s/fold%d/" % (dataset, fold_id))
...
methods = {
    "MooEnsembleSVCbootstrap_c": MooEnsembleSVCbootstrap(base_classifier=base_estimator, crossover_real="real_sbx", crossover_bin="bin_two_point"),
    # "MooEnsembleSVCbootstrap_c": MooEnsembleSVCbootstrap(base_classifier=base_estimator, crossover_real="real_sbx", crossover_bin="bin_two_point")
    # TU DOKONCZ
}
