...
Imbalanced datasets from KEEL:
    - Imbalance ratio higher than 9 - part 2
...
logging.basicConfig(filename='textinfo/experiment2_9higher_part2.log', filemode="a", format='%(asctime)s - %(levelname)s: %(message)s', level='DEBUG')
logging.info("--------------------------------------------------------------------------------")
...
def compute(dataset_id, dataset):
    ...
    try:
        ...
        dataset_path = "datasets/9higher_part2/" + dataset + ".dat"
        ...
        for clf_id, clf_name in enumerate(methods):
            for metric_id, metric in enumerate(metrics_alias):
                # Save metric results
                filename = "results/experiment2_9higher_part2/raw_results/%s/%s/%s.csv" % (metric, dataset, clf_name)
                if not os.path.exists("results/experiment2_9higher_part2/raw_results/%s/%s/" % (metric, dataset)):
                    os.makedirs("results/experiment2_9higher_part2/raw_results/%s/%s/" % (metric, dataset))
                np.savetxt(fname=filename, fmt="%f", X=scores[metric_id, clf_id, :])
            # Save diversity results
            filename = "results/experiment2_9higher_part2/diversity_results/%s/%s.csv" % (dataset, clf_name)
            if not os.path.exists("results/experiment2_9higher_part2/diversity_results/%s/" % (dataset)):
                os.makedirs("results/experiment2_9higher_part2/diversity_results/%s/" % (dataset))
            ...
            for fold_id in range(n_folds):
                for sol_id in range(n_rows_p):
                    if (pareto_solutions[clf_id, fold_id, sol_id, 0] != 0.0) and (pareto_solutions[clf_id, fold_id, sol_id, 1] != 0.0):
                        filename_pareto = "results/experiment2_9higher_part2/pareto_raw/%s/%s/fold%d/sol%d.csv" % (dataset, clf_name, fold_id, sol_id)
                        if not os.path.exists("results/experiment2_9higher_part2/pareto_raw/%s/%s/fold%d/" % (dataset, clf_name, fold_id)):
                            os.makedirs("results/experiment2_9higher_part2/pareto_raw/%s/%s/fold%d/" % (dataset, clf_name, fold_id))
