...
logging.basicConfig(filename='experiment4_9higher_part3.log', filemode="a", format='%(asctime)s - %(levelname)s: %(message)s', level='DEBUG')
logging.info("--------------------------------------------------------------------------------")
    ...
    try:
        ...
        for clf_id, clf_name in enumerate(methods):
            for metric_id, metric in enumerate(metrics_alias):
                # Save metric results
                filename = "results/experiment4_9higher_part3/raw_results/%s/%s/%s.csv" % (metric, dataset, clf_name)
                if not os.path.exists("results/experiment4_9higher_part3/raw_results/%s/%s/" % (metric, dataset)):
                    os.makedirs("results/experiment4_9higher_part3/raw_results/%s/%s/" % (metric, dataset))
                np.savetxt(fname=filename, fmt="%f", X=scores[metric_id, clf_id, :])
            # Save diversity results
            filename = "results/experiment4_9higher_part3/diversity_results/%s/%s.csv" % (dataset, clf_name)
            if not os.path.exists("results/experiment4_9higher_part3/diversity_results/%s/" % (dataset)):
                os.makedirs("results/experiment4_9higher_part3/diversity_results/%s/" % (dataset))
