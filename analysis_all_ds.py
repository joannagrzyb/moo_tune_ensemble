...
def horizontal_bar_chart():
    for metric_id, metric in enumerate(metrics_alias):
        ...
        # Save plot
        filename = "results/experiment_server/experiment_all_ds/plot_bar/bar_%s" % (metric)
        if not os.path.exists("results/experiment_server/experiment_all_ds/plot_bar/"):
            os.makedirs("results/experiment_server/experiment_all_ds/plot_bar/")
...
# # Wilcoxon ranking - statistic test for methods: SEMOOS vs all
pairs_metrics_multi(method_names=methods_alias, data_np=data_np, experiment_name="experiment_server/experiment_all_ds", dataset_names=datasets, metrics=metrics_alias, filename="ex_ranking_plot", ref_method=methods_alias[0])

# SEMOOSb vs all
pairs_metrics_multi(method_names=methods_alias, data_np=data_np, experiment_name="experiment_server/experiment_all_ds", dataset_names=datasets, metrics=metrics_alias, filename="ex_ranking_plot", ref_method=methods_alias[1])

# SEMOOSbp vs all
pairs_metrics_multi(method_names=methods_alias, data_np=data_np, experiment_name="experiment_server/experiment_all_ds", dataset_names=datasets, metrics=metrics_alias, filename="ex_ranking_plot", ref_method=methods_alias[2])
